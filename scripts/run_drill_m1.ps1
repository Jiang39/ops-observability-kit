...
$evidenceDir = Join-Path (Join-Path $repoRoot $RuntimeDir) "evidence\\m1"
...
$errorCount = @($matched | Where-Object { $_.level -eq "error" }).Count
$summary = [pscustomobject]@{
  traceId = $traceId
  total_json_logs = $jsonLines.Count
  matched_trace_logs = $matched.Count
  error_logs = $errorCount
  services = (($matched | Select-Object -ExpandProperty service -Unique) -join ",")
